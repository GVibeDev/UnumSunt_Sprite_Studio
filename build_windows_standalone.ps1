...
Write-Host '=== Unum Sunt Sprite Studio R5c4a - Windows Standalone Core ==='
Write-Host "Build runtime ufficiale: $BuildPythonLabel"
Write-Host ''

if (-not [Environment]::Is64BitOperatingSystem) {
    throw 'R5c4a supporta esclusivamente Windows x64.'
}
...
& $python tools\write_release_manifest.py $distDir (Join-Path $distDir 'RELEASE_MANIFEST_R5c4a.json')
if ($LASTEXITCODE -ne 0) { throw 'Impossibile generare il release manifest.' }

$releaseDir = Join-Path $PSScriptRoot 'release'
New-Item -ItemType Directory -Force $releaseDir | Out-Null
$zipPath = Join-Path $releaseDir 'UnumSunt_Sprite_Studio_R5c4a_Windows_x64_Standalone.zip'
$hashPath = Join-Path $releaseDir 'UnumSunt_Sprite_Studio_R5c4a_Windows_x64_Standalone_SHA256.txt'
...
Write-Host 'Il runtime AI resta esterno al bundle Core ed è gestito dal Runtime Manager R5c4a.'
