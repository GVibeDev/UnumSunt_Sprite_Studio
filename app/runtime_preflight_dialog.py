...
class RuntimePreflightDialog(QDialog):
    def __init__(self, parent=None) -> None:
        ...
        self.setWindowTitle("Verifica runtime AI locale · R5c4a")
        self.resize(980, 700)
    ...
    def _build_ui(self) -> None:
        ...
        intro = QLabel(
            ...
            "R5c4a NON applica soglie minime a modello GPU, VRAM o RAM."
        )
        ...
        contract = QLabel(
            f"Contratto corrente: CUDA >= {self.plan.minimum_reported_cuda} · toolkit WanGP consigliato {self.plan.recommended_toolkit} · "
            f"Python {self.plan.python_version} · PyTorch {self.plan.pytorch_version}"
        )
        ...
        self.component_table.setHorizontalHeaderLabels(["Componente", "Destinazione", "Download", "Installato", "Temp", "Tipo"])
        ...
        self.summary_label = QLabel("PRE-FLIGHT NON ESEGUITO")
        ...
        self.check_table.setHorizontalHeaderLabels(["Stato", "Controllo", "Dettaglio"])
        ...
        buttons = QHBoxLayout()
        run_btn = QPushButton("Esegui verifica")
        run_btn.clicked.connect(self.run_check)
        buttons.addWidget(run_btn)
        self.save_btn = QPushButton("Salva report JSON…")
    ...
    def _populate_plan(self) -> None:
        ...
        for row, component in enumerate(self.plan.components):
            values = [
                ...
                "STIMA" if component.estimate else "CONFERMATO",
            ]
    ...
    def _browse(self, edit: QLineEdit) -> None:
        ...
        selected = QFileDialog.getExistingDirectory(self, "Seleziona cartella", initial)
        if selected:
    ...
    def run_check(self) -> None:
        ...
        try:
            ...
            QMessageBox.critical(self, "Preflight fallito", str(exc))
            return
        config.save()
        self.config = config
        self._report = report
    ...
    def save_report(self) -> None:
        ...
        path, _ = QFileDialog.getSaveFileName(
            ...
            str(Path.home() / "unum_sunt_runtime_preflight_R5c4a.json"),
            "JSON (*.json)",
        )
        if not path:
            return
