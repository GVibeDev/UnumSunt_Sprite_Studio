...
class MaintenanceR5c6aTests(unittest.TestCase):
    ...
    def test_status_report_exposes_ownership(self) -> None:
        ...
        self.assertTrue(any("external" in warning.lower() for warning in report.warnings))

    def test_setup_preserves_same_app_id_and_supports_cleanup_choices(self) -> None:
        root = Path(__file__).resolve().parents[1]
        script = (root / "installer" / "UnumSuntSpriteStudio_R5c8.iss").read_text(encoding="utf-8")
    ...
    def test_build_script_targets_r5c6_setup(self) -> None:
        ...
        self.assertIn("installer\\UnumSuntSpriteStudio_R5c8.iss", text)
        self.assertIn("UnumSunt_Sprite_Studio_R5c8_Setup_x64.exe", text)
