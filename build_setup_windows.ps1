...
Write-Host '=== Unum Sunt Sprite Studio R5c6a - Windows Setup Bootstrapper ==='
...
$installerScript = Join-Path $PSScriptRoot 'installer\UnumSuntSpriteStudio_R5c6a.iss'
if (-not (Test-Path $installerScript)) { throw 'Script Inno Setup R5c6a non trovato.' }

$installerOut = Join-Path $PSScriptRoot 'release\installer'
New-Item -ItemType Directory -Force $installerOut | Out-Null
Remove-Item -Force (Join-Path $installerOut 'UnumSunt_Sprite_Studio_R5c6a_Setup_x64.exe') -ErrorAction SilentlyContinue
Remove-Item -Force (Join-Path $installerOut 'UnumSunt_Sprite_Studio_R5c6a_Setup_x64_SHA256.txt') -ErrorAction SilentlyContinue
...
$setupExe = Join-Path $installerOut 'UnumSunt_Sprite_Studio_R5c6a_Setup_x64.exe'
...
$hashFile = Join-Path $installerOut 'UnumSunt_Sprite_Studio_R5c6a_Setup_x64_SHA256.txt'
"$hash  $(Split-Path $setupExe -Leaf)" | Set-Content -Encoding ascii $hashFile

Write-Host ''
Write-Host 'SETUP R5c6a COMPLETATO'
